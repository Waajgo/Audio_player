import { el, mount } from 'redom';
import { TrackListType, TrackType } from "@/types/types";
import { Track } from "@/model/Track";
import { trackItemView } from "@/view/components/trackItemView";
import calendar from "@/assets/calendar.svg";
import clock from "@/assets/clock.svg";

const mokDataList: TrackListType = [
  {
    id: 1,
    title: "Eternal Sunset",
    artist: "Skyline Sounds",
    duration: 9.36,
    size_mb: 8.57,
    encoded_audio:
      "QXVkaW8gZGF0YSBmb3IgRXRlcm5hbCBTdW5zZXQgYnkgU2t5bGluZSBTb3VuZHM=",
  },
  {
    id: 2,
    title: "City Nights",
    artist: "Urban Beats",
    duration: 4.24,
    size_mb: 3.88,
    encoded_audio: "QXVkaW8gZGF0YSBmb3IgQ2l0eSBOaWdodHMgYnkgVXJiYW4gQmVhdHM=",
  },
  {
    id: 3,
    title: "Ocean Breeze",
    artist: "Deep Wave",
    duration: 3.93,
    size_mb: 3.6,
    encoded_audio: "QXVkaW8gZGF0YSBmb3IgT2NlYW4gQnJlZXplIGJ5IERlZXAgV2F2ZQ==",
  },
  {
    id: 4,
    title: "Morning Dew",
    artist: "Fresh Air",
    duration: 8.47,
    size_mb: 7.75,
    encoded_audio: "QXVkaW8gZGF0YSBmb3IgTW9ybmluZyBEZXcgYnkgRnJlc2ggQWly",
  },
  {
    id: 5,
    title: "Starlit Road",
    artist: "Cosmic Rhythms",
    duration: 2.2,
    size_mb: 2.01,
    encoded_audio:
      "QXVkaW8gZGF0YSBmb3IgU3RhcmxpdCBSb2FkIGJ5IENvc21pYyBSaHl0aG1z",
  },
  {
    id: 6,
    title: "Midnight Escape",
    artist: "Nightfall",
    duration: 5.23,
    size_mb: 4.77,
    encoded_audio:
      "QXVkaW8gZGF0YSBmb3IgTWlkbmlnaHQgRXNjYXBlIGJ5IE5pZ2h0ZmFsbA==",
  },
  {
    id: 7,
    title: "Electric Heart",
    artist: "Volt Sparks",
    duration: 6.85,
    size_mb: 6.33,
    encoded_audio:
      "QXVkaW8gZGF0YSBmb3IgRWxlY3RyaWMgSGVhcnQgYnkgVm9sdCBTcGFya3M=",
  },];
const mokData: TrackType =  {
  id: 1,
  title: "Eternal Sunset",
  artist: "Skyline Sounds",
  duration: 9.36,
  size_mb: 8.57,
  encoded_audio:
    "QXVkaW8gZGF0YSBmb3IgRXRlcm5hbCBTdW5zZXQgYnkgU2t5bGluZSBTb3VuZHM=",
};

function mokView(): HTMLElement {
  const mokTrack = new Track(mokData);
  return trackItemView(mokTrack);
}

export function trackListView(): HTMLTableElement {
  const tableHeaderEl: HTMLTableSectionElement = el('thead', { class: 'track-table__header'}, [
    el('th', '№'),
    el('th', 'Название'),
    el('th', 'Альбом'),
    el('th', el('sl-icon', { class: 'track-table__icon', src: calendar })),
    el('th', el('sl-icon', { class: 'track-table__icon', src: clock })),
  ]);
  const tableBodyEl: HTMLTableSectionElement = el('tbody',  { class: 'track-table__body'});
  const tableEl: HTMLTableElement = el('table', { className: 'tracks__list track-table'}, [
    tableHeaderEl,
    tableBodyEl,
  ]);

  mount(tableBodyEl, mokView());
  return tableEl;
}



// data.forEach(() => {})

